...
Metrics reported per method: route distance, MAKESPAN (Tmax, reusing
run_experiments_C._makespan_h), and fallback/effective distance
(simulator.py, deterministic simulate_solution -- evaluation still uses the
real, non-stochastic simulator so all three methods are judged on the same
fixed yardstick regardless of which one used the stochastic model at
build/training time).
    ...
    if all_rows:
        ...
        for phase, label in (('low', 'LOW OCCUPANCY'), ('high', 'HIGH SATURATION')):
            ...
            for label2, key in (('Time alone   (d_time vs std)   ', 'time_vs_std'),
                ...
                print(f"    {label2}: effective {eff_str}  |  fallback {fb_str}  |  {nfb_str}  |  {ms_str}")
        print(f"\n{'='*88}")
